...
\begin{tikzpicture}
    ...
    % instead of `x'
    % \addplot [domain=0:720, samples=720, red, very thick] {e^(-x*tan(60))};
    % use `rad(x)'
    \addplot [domain=0:720, samples=720, red, very thick] {e^(-rad(x)*tan(60))};
    \addplot [domain=-4:4, samples=100, black, thick, dotted, data cs=cart] {-x/sqrt(3) + 1/sqrt(3)};
    \end{polaraxis}
    % my code
    \begin{scope}[scale=5, yshift=8em]
